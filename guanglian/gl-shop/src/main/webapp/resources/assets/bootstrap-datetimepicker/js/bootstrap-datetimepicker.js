...
!function( $ ) {
	...
	Datetimepicker.prototype = {
		...
		//秒选择时间
		msseond:function(e){
			alert(this.picker.find('select').val());
		},
		hide: function(e){
		...
		fill: function() {
			...
			for(var i=0;i<60;i+=this.minuteStep) {
			 ...
			 addSec = /,s{1,2},/.test(',' + this.format.parts.join(',') + ','), sSec = addSec ? '<select id="msseond" style="width:100%">' : '';
			 this.addSec = addSec;
			 if (addSec) {
			...
			// var thatpicker=this.picker;
			 if(addSec){
				 this.picker.find('select').change(function(e){
					var year    = pickdi.viewDate.getUTCFullYear(),
					month   = pickdi.viewDate.getUTCMonth(),
					day     = pickdi.viewDate.getUTCDate(),
					hours   = pickdi.viewDate.getUTCHours(),
					minutes = pickdi.viewDate.getUTCMinutes(),
					//seconds = this.viewDate.getUTCSeconds();
					seconds =  $('#msseond').val(); 
					pickdi._setDate(UTCDate(year, month, day, hours, minutes, seconds, 0));
					pickdi.fill();
					if (pickdi.autoclose) {
						pickdi.hide();
				 }
		...
		click: function(e) {
			if (e.target.tagName == 'SELECT' ) {
				//e.preventDefault();
				//alert(this.picker.find('select').val());
				//this.picker.find('select').trigger("change");
				//alert(this.picker.find('select').val());
				pickdi=this;
			...
			if (e.target.tagName == 'SELECT' || e.target.tagName == 'OPTION'){
				this.viewDate.setUTCSeconds(this.picker.find('select').val());
