...
Dropify.prototype.createElements = function()
{
    ...
    if (defaultFile.trim() != '') {
        this.setFilename(defaultFile);//设置:图片格式
...
Dropify.prototype.readUrl = function(input)
{
    if (input.files && input.files[0]) {
    	...
    	//this.getI
       // this.setFilename(getImageName(this.input.val()));
        if (this.checkFileSize()) {
            ...
            //saveImage(this.input.attr("id"),this.filename);
        } else {
...
Dropify.prototype.setPreview = function(src)
{
    ...
    if (this.isImage() === true) {
        $('<img />').attr('src', src).appendTo(render).css({
        	'background-color':'red'
        	
        });
        
    } else {
    	 if(src="http://localhost:8080/gl-shop/static/DEFAULT/LOGO/"){
    		 return true;
    	 }else{
    		 alert("对不起，您添加的图片格式不正确，请输入['png', 'jpg', 'jpeg', 'gif', 'bmp']格式");
    	    return false; 
    	 }
    }
...
Dropify.prototype.maxFileSizeToByte = function()
{
    ...
    if (this.settings.maxFileSize <=2) {
        var unit  = 'MB',
            kb    = 1024,
            mb    = kb * 1024,
            gb    = mb * 1024;
