		...
		<header class="header white-bg text-center">
              ...
              <c:choose>
              	<c:when test="${sessionScope.ADMIN_USER!=null &&  sessionScope.ADMIN_USER.uAgent!=null}">
              		<a href='<c:url value="/water/csite/phonefrom/chome.html"/>' class="logo visible-xs" ><i class="theme-color fa fa-home"></i>&nbsp;&nbsp;</a>
              	</c:when>
              	<c:otherwise>
              		<a href='<c:url value="/water/csite/pcfrom/chome.html"/>' class="logo visible-xs" ><i class="theme-color fa fa-home"></i>&nbsp;&nbsp;</a>
              	</c:otherwise>
              </c:choose>
			<!--logo start-->
			...
			<div class="horizontal-menu navbar-collapse collapse text-left pull-right font-size16">
                  <ul class="nav navbar-nav">
                      <c:forEach items="${requestScope.WATERMENULIST}" var="menu" varStatus="status">
		...
		<section id="main-content">
			<section class="wrapper">
				...
				<aside>
					<c:choose>
						<c:when test="${pid==-1 }">
							<jsp:include page="/pages/water/csite/projecttree.jsp" />
						</c:when>
						<c:otherwise>
							<jsp:include page="/pages/water/csite/left.jsp" />
						</c:otherwise>
					</c:choose>
				</aside>
				<!--sidebar end-->
