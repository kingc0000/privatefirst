...
<body>
	...
	<section id="container" class="full-width">
                      	...
                      	<sec:authorize access="hasRole('${menu.role}') and fullyAuthenticated">
                      		<c:choose>
                      			<c:when test="${fn:length(menu.menus)>0 }">
                      				<li class="dropdown ">
                      					<a data-toggle="dropdown" data-hover="dropdown" class="dropdown-toggle  needsclick <c:if test="${activeMenus[menu.code]!=null}"> active</c:if>" href="#"><span>${menu.name}</span><b class=" fa fa-angle-down"></b></a>
		...
		<!--main content end-->
		<jsp:include page="/layout/water/systemfooter.jsp" />
	</section>
<!-- Material button -->
