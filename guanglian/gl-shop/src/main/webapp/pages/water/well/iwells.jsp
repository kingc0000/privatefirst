                      ...
                      <div class="panel-body">
                      	<form:form cssClass="form-horizontal" role="form" commandName="oWell" id="storeform">
							...
							<div class="form-group">
                               ...
                               <div class="form-group">
                                   <label class="col-lg-2 col-sm-2 control-label control-required">回灌井内水位告警阈值</label>
                                   <div class="col-lg-10">
                                       <form:input cssClass="form-control required" type="number" path="pressure" id="pressure" />
                                       <span class="help-block"> <form:errors path="pressure" cssClass="error" /></span>
                                   </div>
                               </div>
                               <div class="form-group">
                                   <label class="col-sm-2 control-label">井内水位采集转换公式</label>
                                   <div class="col-sm-6">
	                                   ...
	                                   <div class="col-lg-10">
	                                       <form:select type="select" cssClass="form-control" path="dataStatus" id="dataStatus"  >
											 <form:option value="0" selected="selected" >正常</form:option>
												...
												<form:option value="2">井内水位告警</form:option>
						...
						<form:hidden path="rPressure" id="rPressure" />
            	...
            	<div class="container-fluid form-horizontal dataselect">
	            	<div class="row">
	            		...
	            		<div class="col-xs-2 col-sm-3 col-md-4 singlechart">
	            			<button onclick="getHistoryLines(2)" class="btn btn-theme"><i class="fa fa-search"></i></button>
	            		</div>
	            		<div class="col-xs-12 col-sm-3 col-md-4 comparechart">
	            			<button onclick="getHistoryLines('21')" class="btn btn-theme"><i class="fa fa-search"></i>流量比较</button>
		            		<button onclick="getHistoryLines('22')" class="btn btn-warning"><i class="fa fa-search"></i>井内水位比较</button>
	            		</div>
	            	</div>
            	</div>
            	<div class="panel-body">
...
<jsp:include page="/pages/water/import/imporiwell.jsp" />
...
function goback(){
                        ...
                        {"aTargets":[7],"mRender":function(data,type,full){
							...
							default:
                             ...
                             {"aTargets":[11],"mRender":function(data,type,full){
                            	 var reStr='<a class="btn btn-info btn-xs tooltips" data-placement="left" data-container="body" data-original-title="数据列表" href="'+_context+'/water/idata/list.html?pid='+full.id+'" ><i class="fa fa-list"></i></a>&nbsp;';
	...
	function erprint(){
		...
		window.open('<c:url value="/admin/iwell/print.html"/>'+'?ids='+ids, "_blank");		
	}
	function jssubmit(){
		var form = $("#storeform");
		form.validate({
	...
	function dolistafterfn(oTable, nRow){
		...
		if(fname!=null && fname.length >0){
			$.each(fname,function(index,img){
				addImage(img.name,img.jpeg,img.id,"IWELL_DIGITAL");
			});
		}
