	...
	<div class="col-lg-12 col-sm-12">
				<section class="panel" >
                      ...
                      <div class="panel-body">
                      	<form:form cssClass="form-horizontal" role="form" commandName="oWell" id="storeform">
							...
							<div class="form-group">
                               ...
                               <div class="form-group">
                                   ...
                                   <div class="col-lg-10">
                                       <form:input cssClass="form-control required" type="number" path="waterMeasurement" id="waterMeasurement" />
                                       <span class="help-block"> <form:errors path="waterMeasurement" cssClass="error" /></span>
                                   </div>
                               </div>
                               <div class="form-group">
                               ...
                               <div class="form-group">
                                   <label class="col-lg-2 col-sm-2 control-label control-required">水温告警阈值</label>
                                   <div class="col-lg-10">
                                       <form:input cssClass="form-control required" type="number" path="waterTemperature" id="waterTemperature" />
                                       <span class="help-block"> <form:errors path="waterMeasurement" cssClass="error" /></span>
                                   </div>
                               </div>
                               <div class="form-group">
                                   <label class="col-sm-2 control-label">水温采集转换公式</label>
                                   <div class="col-sm-6">
                                       <form:input cssClass="form-control" path="formula3" id="formula3" />
                                   </div>
                                   <div class="col-sm-4 control-label">请用x代表采集数值</div>
                               </div>
                                <div class="form-group">
	                                   ...
	                                   <div class="col-lg-10">
	                                       <form:select type="select" cssClass="form-control" path="dataStatus" id="dataStatus"  >
											 <form:option value="0" selected="selected" >正常</form:option>
												<form:option value="1">水位告警</form:option>
												<form:option value="2">水温告警</form:option>
						...
						<form:hidden path="rTemperature" id="rTemperature" />
            	...
            	<div class="container-fluid form-horizontal dataselect">
	            	<div class="row">
	            		...
	            		<div class="col-xs-2 col-sm-3 col-md-4 singlechart">
	            			<button onclick="getHistoryLines(1)" class="btn btn-theme"><i class="fa fa-search"></i></button>
	            		</div>
	            		<div class="col-xs-12 col-sm-3 col-md-4 comparechart">
	            			<button onclick="getHistoryLines('11')" class="btn btn-theme"><i class="fa fa-search"></i>水位比较</button>
		            		<button onclick="getHistoryLines('12')" class="btn btn-warning"><i class="fa fa-search"></i>温度比较</button>
	            		</div>
	            	</div>
            	</div>
            	<div class="panel-body">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">转换抽水井</h4>
            </div>
            <div class="modal-body">
				确认是否将该井转换成抽水井，如果确认，请耐心等待完成该井采集数据的迁移！
            </div>
            <div class="modal-footer">
                <button data-dismiss="modal" class="btn btn-default" type="button">取消</button>
                <button class="btn btn-warning" type="button" onclick="doSwitch()">确定</button>
            </div>
        </div>
    </div>
</div>
 <jsp:include page="/common/getPointFromMap.jsp" />
<jsp:include page="/pages/water/import/imporowell.jsp" />
...
var groupid=-1;
                ...
                "aoColumnDefs": [
                            ...
                            {"aTargets":[7],"mRender":function(data,type,full){
                             ...
                             {"aTargets":[12],"mRender":function(data,type,full){
                            	 var reStr='<a class="btn btn-info btn-xs tooltips" data-placement="left" data-container="body" data-original-title="数据列表" href="'+_context+'/water/odata/list.html?pid='+full.id+'" ><i class="fa fa-list"></i></a>&nbsp;';
                            	...
                            	<c:if test="${hasRight==true }">
                            		if(full.powerStatus=="0"||full.powerStatus!="1"){
	                               	...
	                               	reStr+='<a class="btn btn-warning btn-xs tooltips" data-placement="left" data-container="body" data-original-title="转观测井" href="javascript:;" onclick="switchWell('+data+')"><i class="fa fa-expand"></i></a>&nbsp;';
                				</c:if>
	...
	function erprint(){
		...
		window.open('<c:url value="/admin/owell/print.html"/>'+'?ids='+ids, "_blank");		
	}
	function jssubmit(){
			if(parseFloat($('#waterDwon').val())> parseFloat($('#waterMeasurement').val())){
	...
	function dolistafterfn(oTable, nRow){
		...
		if(fname!=null && fname.length >0){
			$.each(fname,function(index,img){
				addImage(img.name,img.jpeg,img.id,"OWELL_DIGITAL");
			});
		}
