            	...
            	<div class="container-fluid form-horizontal dataselect">
	            	<div class="row">
	            		...
	            		<div class="col-xs-2 col-sm-3 col-md-4 singlechart">
	            			<button onclick="getHistoryLines(0)" class="btn btn-theme"><i class="fa fa-search"></i></button>
	            		</div>
	            		<div class="col-xs-12 col-sm-3 col-md-4 comparechart">
	            			<button onclick="getHistoryLines('01')" class="btn btn-theme"><i class="fa fa-search"></i>流量比较</button>
		            		<button onclick="getHistoryLines('02')" class="btn btn-warning"><i class="fa fa-search"></i>水位比较</button>
	            		</div>
	            	</div>
            	</div>
            	<div class="panel-body">
...
<div class="modal fade" id="switchModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">转换观测井</h4>
            </div>
            <div class="modal-body">
				确认是否将该井转换成观测井，如果确认，请耐心等待完成该井采集数据的迁移！
            </div>
            <div class="modal-footer">
 ...
 <jsp:include page="/pages/water/import/imporpwell.jsp" />
               ...
               "aoColumnDefs": [
                           ...
                           {"aTargets":[8],"mRender":function(data,type,full){
                            ...
                            {"aTargets":[12],"mRender":function(data,type,full){
                           	 var reStr='<a class="btn btn-info btn-xs tooltips" data-placement="left" data-container="body" data-original-title="数据列表" href="'+_context+'/water/pdata/list.html?pid='+full.id+'" ><i class="fa fa-list"></i></a>&nbsp;';
...
function erprint(){
	...
	window.open('<c:url value="/admin/pwell/print.html"/>'+'?ids='+ids, "_blank");		
}
...
function dolistafterfn(oTable, nRow){
	...
	if(fname!=null && fname.length >0){
		$.each(fname,function(index,img){
			addImage(img.name,img.jpeg,img.id,"PWELL_DIGITAL");
		});
	}
