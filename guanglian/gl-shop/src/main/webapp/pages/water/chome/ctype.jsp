...
<script>
	...
	function drawbin(result,index,fisrt,width,zone){
        ...
        str+='<div class="col-xs-6 text-center"><div><canvas id="pie-chart'+index+'" width="'+width+'" height="'+width+'" style="display: inline-block; vertical-align: top;"></canvas></div></div>';
        str+='<div class="col-xs-6 text-center"><div><canvas id="pie-total'+index+'" width="'+width+'" height="'+width+'" style="display: inline-block; vertical-align: top;"></canvas></div></div>';
        var total=result.down+result.run+result.srun+result.over;
        if(fisrt==false){
        	str+='<div class="col-xs-12 text-center" style="font-size:22px;padding:0px;"><a class="list-group-item " href="'+_context+'/water/csite/${mediafrom}/cprojects.html?ctype='+result.val+'&zone='+zone+'">'+result.name+'('+total+')</a></div>';
        }else{
