...
<link href='<c:url value="/resources/css/marquee.css" />'
	rel="stylesheet">
...
<div class="cmarquee">
	<div class="marquee-sibling" style="top: 7px;">
		<button class="btn btn-img btn-selected tooltips maptools "
			style="border-radius:4px 4px 0 0;" data-placement="top" data-container="body"
			data-original-title="图例" onclick="showtools()">
			<i class="fa fa-anchor"></i>
		</button>
		<span class="chartBtn">
			<button class="btn btn-img btn-selected tooltips maptools"
				data-placement="top" data-container="body"
				data-original-title="项目图纸" onclick="getDrawing()">
				<i class="fa fa-building-o"></i>
			</button>
			<button class="btn btn-img btn-selected tooltips maptools"
				data-placement="top" data-container="body"
				data-original-title="图表数据" onclick="showdata()">
				<i class="fa fa-bar-chart-o"></i>
			</button>
			<button class="btn btn-img btn-selected tooltips maptools"
				data-placement="top" data-container="body"
				data-original-title="我的评论" onclick="toPreview()">
				<i class="fa fa-comment"></i>
			</button>
			<button class="btn btn-img btn-selected tooltips maptools"
				data-placement="top" data-container="body"
				data-original-title="每日信息" onclick="getConclusion()">
				<i class="fa fa-calendar"></i>
			</button>
			<c:if test="${hasRight}">
			<button class="btn btn-img btn-selected tooltips maptools"
				data-placement="top" data-container="body"
				data-original-title="施工日志" onclick="editDaily()">
...
<div class="cmarquee">
	<div class="marquee-sibling" style="top: 39px;">
		<div class="cmaptool" style="display:none;text-align: left; line-height: 12px; background-color: #6699ff; width: 27px; z-index: 100;">
			<button class="btn btn-img btn-selected tooltips well-icon pwell "
				onclick="setlegend('pwell','pmark')" data-container="body"
				data-placement="top" data-original-title="降水井图例"></button>
			<br>
			<button class="btn btn-img btn-selected tooltips well-icon dewell"
				onclick="setlegend('dewell','demark')" data-container="body"
				data-placement="top" data-original-title="疏干井图例"></button>
			<br>
			<button class="btn btn-img btn-selected tooltips well-icon owell"
				onclick="setlegend('owell','omark')" data-container="body"
				data-placement="top" data-original-title="观测井图例"></button>
			<br>
			<button class="btn btn-img btn-selected tooltips well-icon iwell"
				onclick="setlegend('iwell','imark')" data-container="body"
				data-placement="top" data-original-title="回灌井图例"></button>
			<br>
			<button class="btn btn-img btn-selected tooltips well-icon ewell"
				onclick="setlegend('ewell','emark')" data-container="body"
				data-placement="top" data-original-title="环境监测图例"></button>
			<br>
			<button class="btn btn-img btn-selected tooltips well-icon camera"
				onclick="setlegend('camera','camera')" data-container="body"
				data-placement="top" data-original-title="摄像头图例"></button>
		</div>
	</div>
</div>
		...
		<div class="modal-content">
			<div class="modal-header">
				...
				<h4 class="modal-title">
					<span id="modal-title" class="projectname text-warning"></span>
				</h4>
			</div>
		...
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal"
					aria-hidden="true">&times;</button>
				<h4 class="modal-title">
					<span class="projectname text-warning"></span>数据图
				</h4>
			</div>
...
<script>
	...
	function getDrawing() {
		$.post(
				...
				function(imgs) {
					...
					str1 += '</div>';
					$.getScript(
									'<c:url value="/resources/js/slide.js" />',
									function() {
										baguetteBox
												.run(
														'.gallery',
	...
	function getWarning() {
		...
		$('#mitems li')
				.each(
						function() {
							if ($(this).attr('id')) {
								strli += '<li><a href="javascript:;" onclick="toProject('
										+ $(this).attr('id') + ')">';
	...
	function toPreview() {
		window.location.href = '<c:url value="/water/preview/review.html?cid="/>'
				+ pid;
	}
					...
					.each(
							...
							function(i, data) {
								...
								var myIcon = new BMap.Icon(
										'<c:url value="/resources/img/water/checks.png"/>',
										new BMap.Size(30, 23), {
											imageOffset : new BMap.Size(-30
													* sts, 0)
										});
								...
								var content = '<form id="pform" action="'+_context+'/water/pdata/savemap.shtml" autocomplete="off">名称：'
										+ data.name
										+ '<br>运行/采集状态：'
										+ getCstatus(data.powerStatus,
												data.dataStatus);
								content += '<span class="mapedit" style="display:none;"><br><span class="mapedit" >上次累积流量:</span> <input class="mapedit lastAccu" onkeyup="autoc(&apos;pform&apos;)" style="width:77px;border: 1px solid #e2e2e4;" type="text" name="lastAccu" value="'
										+ lastwater + '"/>';
								...
								content += '<br>流量值：'
										+ rflow
										+ '  <input class="mapedit flow" readonly="readonly" style="display:none;width:77px;color:#ff6c60;border: 0px solid #e2e2e4;" type="text" name="flow"/><br>'
										+ '流量阈值：' + data.flow + '<br>水位值：'
										+ rwater
								content += '  <input class="mapedit" style="display:none;width:77px;border: 1px solid #e2e2e4;" type="text" name="water"/><br>'
										+ '水位阈值上限：' + data.water;
								content += ' <br>' + '水位阈值下限：' + data.waterDwon;
								content += '<input type="hidden" name="pid" value="'+data.id+'" /></form><br/><button class="btn btn-xs btn-theme" onclick="showLines(\'\', 0, '
										+ data.id
										+ ', true)"><i class="fa fa-bar-chart-o"></i></button>';
								if (hasright) {
									var stbtn = "关闭";
									if (sts == 1) {
										stbtn = "开启";
			...
			$
					.each(
							...
							function(i, data) {
								...
								content += '<span class="mapedit" style="display:none;"><br><span class="mapedit" >上次累积流量:</span> <input class="mapedit lastAccu" onkeyup="autoc(&apos;deform&apos;)" style="width:77px;border: 1px solid #e2e2e4;" type="text" name="lastAccu" value="'
										+ lastwater + '"/>';
								...
								content += '<br>流量值：'+ rflow+ '  <input class="mapedit flow" readonly="readonly" style="display:none;width:77px;color:#ff6c60;border: 0px solid #e2e2e4;" type="text" name="flow"/><br>'
										+ '流量阈值：' + data.flow + '<br>水位值：'+ rwater
								content += '  <input class="mapedit" style="display:none;width:77px;border: 1px solid #e2e2e4;" type="text" name="water"/><br>'
										+ '水位阈值上限：' + data.water;
								content += '<br>' + '水位阈值下限：' + data.waterDwon;
								content += '<input type="hidden" name="pid" value="'+data.id+'" /></form><br/><button class="btn btn-xs btn-theme" onclick="showLines(\'\', 4, '
										+ data.id
										+ ', true)"><i class="fa fa-bar-chart-o"></i></button>';
								if (hasright) {
									var stbtn = "关闭";
									...
									content += '  <button class="btn btn-xs btn-theme" onclick="domapedit(&apos;de&apos;,&apos;'
											+ data.lastDate
											+ '&apos;)">编辑</button>';
	...
	function setOwell(datas, hasright) {
		...
		if (datas != null) {
			...
			$.each(datas,function(i, data) {
								...
								var content = '<form id="oform" action="'+_context+'/water/odata/savemap.shtml" autocomplete="off">名称：'+ data.name+ '<br>运行/采集状态：'+ getCstatus(data.powerStatus,data.dataStatus)+ '<br>水位值：'+ rwater
										+ '  <input type="text" class="mapedit" style="display:none;width:77px;border: 1px solid #e2e2e4;" name="water"/><br>'
										+ '水位阈值上限：' + data.waterMeasurement;
								content += '<br>'
										+ '水位阈值下限：'
										+ data.waterDwon
										+ '<br>水温值：'
										+ rflow
										+ '  <input type="text" class="mapedit" style="display:none;width:77px;border: 1px solid #e2e2e4;" name="temperature"/><br>'
										+ '水温阈值：' + data.waterTemperature;
								content += '  <input type="hidden" name="pid" value="'+data.id+'" /></form><br/><button class="btn btn-xs btn-theme" onclick="showLines(\'\', 1, '
										+ data.id
										+ ', true)"><i class="fa fa-bar-chart-o"></i></button>';
								if (hasright) {
									var stbtn = "关闭";
	...
	function setIwell(datas, hasright) {
		...
		if (datas != null) {
			...
			$.each(datas,function(i, data) {
								...
								content += '<span class="mapedit" style="display:none;"><br><span class="mapedit" >上次累积流量:</span> <input class="mapedit lastAccu" onkeyup="autoc(&apos;iform&apos;)" style="width:77px;border: 1px solid #e2e2e4;" type="text" name="lastAccu" value="'
										+ lastwater + '"/>';
								...
								content += '<br>流量值：'
										+ rwater
										+ ' <input type="text" class="mapedit flow" readonly="readonly" style="display:none;width:77px;color:#ff6c60;border: 0px solid #e2e2e4;" name="flow"/><br>'
										+ '流量阈值：' + data.flow + '<br>井内水位值：'
										+ data.rPressure;
								content += '   <input type="text"  style="display:none;width:77px;border: 1px solid #e2e2e4;" class="mapedit" name="pressure"/><input type="hidden" name="pid" value="'+data.id+'" /><br>'
										+ '井内水位阈值：'
										+ data.pressure
										+ '</form><br/><button class="btn btn-xs btn-theme" onclick="showLines(\'\', 2, '
										+ data.id
										+ ', true)"><i class="fa fa-bar-chart-o"></i></button>';
								if (hasright) {
									var stbtn = "关闭";
									...
									content += '  <button class="btn btn-xs btn-theme" onclick="domapedit(&apos;i&apos;,&apos;'
											+ data.lastDate
											+ '&apos;)">编辑</button>';
			...
			$
					.each(
							...
							function(i, data) {
								...
								var content = '<form id="eform" action="'+_context+'/water/edata/savemap.shtml" autocomplete="off">名称：'
										+ data.name
										+ '<br>运行/采集状态：'
										+ getCstatus(data.powerStatus,
												data.dataStatus)
										+ '<br>监测值：'
										+ rflow
										+ '   <input type="text" style="display:none;width:77px;border: 1px solid #e2e2e4;" class="mapedit" name="data"/><br>'
										+ '监测阈值：' + data.deformData;
								content += '<input type="hidden" name="pid" value="'+data.id+'" /></form>';
								content += '<button class="btn btn-xs btn-theme" onclick="showLines(\'\', 3, '
										+ data.id
										+ ', true)"><i class="fa fa-bar-chart-o"></i></button>';
								if (hasright) {
									var stbtn = "关闭";
									...
									content += '  <button class="btn btn-xs btn-theme" onclick="doaction('
											+ data.id
											+ ',&apos;/water/dmonitor/active.shtml&apos;,&apos;'
											+ stbtn
											+ '该'
											+ data.name
											+ '&apos;)">' + stbtn + '</button>';
																				...
																				.post(
																						...
																						function(
																							...
																							$(
																									"#showimg-overlay")
																									.html(
																											'<div class="full-image"><img src="'+src+'"/><div>');
																							$(
																									"#showimg-overlay")
																									.show();
																							$(
	...
	function warningTitle(datas, title) {
		...
		if (datas != null && datas.length > 0) {
			$('#mitems').append(
					'<li>' + '<i class="fa fa-warning">&nbsp;</i>以下' + title
							+ '有运行故障或者采集告警：' + '</li>');
