	...
	<section class="panel">
			...
			<div class="panel-body " style="outline: none;overflow-y:scroll;" id="no-more-tables" >
				...
				<table class="table  table-bordered">
					...
					<tr>
						<td class="hidden-xs" style="width:20%">时间</td><td data-title="时间" colspan="2"> ${mon.auditSection.dateCreated }</td>
					</tr>
					<tr>
						<td class="hidden-xs" style="width:20%">问题紧急程度</td><td data-title="问题紧急程度" colspan="2">${mon.monRank }</td>
					</tr>
					<tr>
						<td class="hidden-xs" style="width:20%">问题类型</td><td data-title="问题类型" colspan="2">${mon.monType }</td>
					</tr>
					<tr>
						<td class="hidden-xs" style="width:20%">负责人</td><td data-title="负责人" colspan="2">${mon.owner }</td>
					</tr>
					<tr>
						<td class="hidden-xs" style="width:20%">问题处理情况</td><td data-title="问题处理情况" colspan="2">
							<c:choose>
								<c:when test="${mon.monStatus==0}">未处理</c:when>
								<c:otherwise>已处理</c:otherwise>
							</c:choose>
						</td>
					</tr>
					<tr>
						<td class="hidden-xs" style="width:20%">处理人</td><td data-title="处理人" colspan="2">${mon.didman }</td>
					</tr>
					<tr>
						<td class="hidden-xs" style="width:20%">处理意见</td><td data-title="处理意见" colspan="2">${mon.suggest }</td>
					</tr>
						<c:if test="${not empty mon.attach}">
						  <tr >
					  		<td class="hidden-xs" style="width:20%">附件</td>
					  		<td data-title="附件" colspan="2">
					  			<c:forEach items="${mon.attach}" var="imgs">
                       				<div class="page-break" style="margin:0 auto;width:100%;height:100%">
                       					<c:choose>
                       							<c:when test="${imgs.fileType==true }">
                       								<img class="preimg" src='<sm:contentImage imageName="${imgs.fileName}" imageType="MONITOR_PRO"/>'/>
                       							</c:when>
                       							<c:otherwise>
                       								<br>
                       								<a class="tooltips hidden-xs" data-placement="left" data-original-title="下载" href="<c:url value="/files/downloads/${imgs.fileName}?ftype=MONITOR_PRO"/>"><i class="fa fa-download">${imgs.fileName}</i></a>
                       							</c:otherwise>
                       					</c:choose>
                       				</div>	
                       			</c:forEach>	
