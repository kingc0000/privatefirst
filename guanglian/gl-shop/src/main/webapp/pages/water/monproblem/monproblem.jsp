		...
		<section class="panel">
			...
			<div class="panel-body">
				<form:form cssClass="form-horizontal" role="form"
					commandName="monproblem" id="storeform">
...
<script>
$('#pictitle').html("${mentity.name }项目问题");
var mid = '${mid}'; //项目id
							...
							"aoColumnDefs" : [
{
									...
									},
									{
										"aTargets" : [ 6 ],
										"mRender" : function(data, type, full) {
											var s='<a class="preview btn btn-info btn-xs tooltips" data-placement="left" data-container="body" data-original-title="预览" href="<c:url value="/water/monproblem/preview.html?mid='
						...
						if(data!=null && data.length >0){
							for(var i=0;i<data.length;i++){
								if(data[i].fileType==true){
									addImage(data[i].fileName,data[i].fileName,data[i].id,"MONITOR_PRO");
								}else{
									addImage(data[i].fileName,"",data[i].id,"MONITOR_PRO");
								}
	...
	function domanaction(){
		$('#do-modal').modal('hide');
		$('#loading').show();
		$.post('doman.shtml',{"muid":$("#muid").val(), "suggest":$("#suggest").val()}, function(response){
