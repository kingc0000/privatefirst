		...
		<section class="panel">
			...
			<div class="panel-body">
				<form:form cssClass="form-horizontal" role="form"
					commandName="monitoreqip" id="storeform">
					...
					<div class="form-group">
									<label class="col-lg-2 col-sm-2 control-label">关联测点</label>
									<div class="col-lg-10">
										<div>
										<c:forEach var="mpoint" items="${ms}">
											<span class="checkboxs-inline" onclick="setCheckbox(${mpoint.id})">
													<input type="checkbox" id="mps" name="mps" title="${mpoint.id}" onclick="setCheckbox(${mpoint.id})"> &nbsp;<span>${mpoint.pName}</span>
													</span>
										</c:forEach>
										</div>		
									</div>
...
<script>
$('#pictitle').html("${mentity.name }设备信息");
	$(function() {
$('#slist').dataTable({
		...
		$.ajax({
			...
			success : function(response) {
				...
				if(response.length>0){
					$.each($('input[name="mps"]'),function(index,data){
						for(var i=0;i<response.length;i++){
							if($(this).attr("title")==response[i]){
