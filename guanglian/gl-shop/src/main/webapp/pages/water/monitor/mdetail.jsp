...
<link href='<c:url value="/resources/assets/bootstrap-fileupload/bootstrap-fileupload.css" />' rel="stylesheet">
				...
				<form:form cssClass="form-horizontal" role="form"
					commandName="monitor" id="storeform">
					...
					<div class="form-group">
						...
						<div class="col-lg-9 col-sm-9">
							<div class="row" id="wimgs">
								<c:choose>
									<c:when test="${not empty monitor.images }">
										<c:forEach items="${monitor.images}" var="img">
											<div class="imgItem">
												<div class="fileupload fileupload-new col-lg-11 col-sm-11"
													data-provides="fileupload">
													<span class="preview" style="margin-left: 10px;"> <c:choose>
															<c:when test="${not empty img.jpeg }">
																<a class="tooltips" data-placement="left"
																	data-container="body" data-original-title="预览"
																	href="<c:url value="/files/preview/${img.jpeg } "/>"><i
																	class="fa fa-eye">${img.jpeg }</i></a>&nbsp;
                            		</c:when>
															<c:otherwise>
																<a class="tooltips" data-placement="left"
																	data-container="body" data-original-title="预览"
																	href="<c:url value="/files/preview/${img.name } "/>"><i
																	class="fa fa-eye">${img.name }</i></a>&nbsp;
                            		</c:otherwise>
														</c:choose>
														...
														href="<c:url value="/files/downloads/${img.name }"/>"><i
															class="fa fa-download">${img.name }</i></a>&nbsp;
													</span>
												</div>
					...
					<div class="form-group">
						...
						<div class="col-lg-10 col-sm-10 ">
							<div class="date edate input-group">
								...
								path="mBase.pCreated" id="mBase.pEnd" />
...
<script>
$('#pictitle').html("${mentity.name }项目信息");
	$(function(){
		//加载日期控件
		$('.edate').datepicker({
			format : "yyyy-mm-dd",
