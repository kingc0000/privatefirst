		...
		<section class="panel">
			...
			<div class="panel-body">
				<form:form cssClass="form-horizontal" role="form"
					commandName="supaxial" id="storeform">
					<div class="form-group">
					<label class="col-lg-2 col-sm-2 control-label control-required">支撑类型</label>
					<div class="col-lg-10">
						<form:select path="zhiCheng" class="required" items="${blist}" itemLabel="name" itemValue="name"></form:select>
					</div>
				</div>
					<jsp:include page="/common/mBase.jsp"></jsp:include>
					<div class="form-group">
						<label class="col-lg-2 col-sm-2 control-label control-required">报警值</label>
						<div class="col-lg-10">
							<form:input cssClass="form-control required" path="alarmV"
								id="alarmV" />
							<span class="help-block"> <form:errors path="alarmV"
									cssClass="error" /></span>
						</div>
					</div>
...
<script>
	var mid = '${mid}';
	$('#pictitle').html("${mentity.name }支撑轴力");
	$(function() {
		$('#slist')
				.dataTable(
						{
									...
									},
									{
										...
										"mRender" : function(data, type, full) {
											var s = '<a class="btn btn-info btn-xs tooltips" data-placement="left" data-container="body" data-original-title="数据列表" href="'
													+ _context
													+ '/water/supaxialdata/list.html?sid='
													+ full.id
													+ '&mid='
													+ mid
