...
function removeImage(){
	...
	$.ajax({
	  type: 'POST',
	  url: '<c:url value="/water/configuration/removeLogo.shtml"/>',
	  dataType: 'json',
...
<section class="panel">
	<header class="panel-heading"> LOGO配置</header>
                     	 ...
                     	 <c:choose>
                             ...
                             <c:otherwise>
                             	<div class="avatar-view has-preview">
							    	<img class="avatar-img" id="fullImageUrl" style="max-width:100%" src='<sm:contentImage imageName="${logo}" imageType="LOGO"/>'/>
							    </div>
                             </c:otherwise>
                         </c:choose>
                     </div>
...
var avatarObj;
$(function(){
	$('#pictitle').html('Logo管理');
	avatarObj = $('#crop-avatar').cropAvatar({
		removeImgUrl: '<c:url value="/water/configuration/removeLogo.shtml"/>',
        //aspectRatio: 1,
...
function doSave(){
	$('#loading').show();
	/* $("#storeform").submit();  */
	var url = '<c:url value="/water/configuration/savelogo.shtml"/>';
