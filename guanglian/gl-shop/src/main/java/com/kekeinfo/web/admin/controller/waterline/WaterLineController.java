package com.kekeinfo.web.admin.controller.waterline;
...
public class WaterLineController {
	private static final Logger LOGGER = LoggerFactory.getLogger(WaterLineController.class);
	@Autowired
	...
	@RequestMapping(value = "/water/waterline/list.html", method = RequestMethod.GET)
	public String listwaterlines(Model model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String mid = request.getParameter("mid");
		setMenu(model, request);
		WaterLine waterline = new WaterLine();
		model.addAttribute("waterline", waterline);
		//判断用户是否有该项目的编辑权限
		...
		return "admin-waterline";
	}

	@PreAuthorize("hasRole('VIEW-PROJECT')")
	@RequestMapping(value = "/water/waterline/server_processing.shtml", method = { RequestMethod.GET,
			RequestMethod.POST }, produces = " application/json; charset=utf-8")
	public @ResponseBody String getWaterLine(@RequestParam String aoData, HttpServletRequest request,
			HttpServletResponse response) throws JsonGenerationException, JsonMappingException, IOException {
		AjaxResponse resp = new AjaxResponse();
		DataTableParameter dataTableParam = new DataTableParameter(aoData);
		DataTable<MbasePoint<WaterLine>> dt = new DataTable<MbasePoint<WaterLine>>();
		String mid = request.getParameter("mid");
		String mno =request.getParameter("markNO");
		try { // 指定根据什么条件进行模糊查询
			...
			if(StringUtils.isNotBlank(mno)){
				where.add(new Object[]{"markNO", mno,"5"});
			}
			
			Entitites<MbasePoint<WaterLine>> list = waterlineService.getPageListByAttributesLike(attributes, dataTableParam.getsSearch(), Long.valueOf(dataTableParam.getiDisplayLength()).intValue(), 
			...
			LOGGER.error("Error while paging waterlines", e);
	...
	@RequestMapping(value = "/water/waterline/save.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody String saveWaterLine(@ModelAttribute("waterline") WaterLine waterline, Model model,
			HttpServletRequest request, HttpServletResponse response) {
		AjaxResponse resp = new AjaxResponse();
		try {
			waterlineService.saveOrUpdate(waterline);
				MbasePoint<WaterLine> bdata =  waterlineService.getById(waterline.getId(), MPointEnumType.WaterLine);
				MDbasePoint<WaterLineData> waterlinedata = waterlinedataService.getEqualsHeightData(bdata.getInitHeight(), waterline.getId(), MPointEnumType.WaterLine);
				if(waterlinedata!=null) {
					waterlinedata.setInitHeight(waterline.getInitHeight());
					waterlinedataService.update(waterlinedata);
				}
		} catch (ServiceException e) {
			LOGGER.error("Error while save WaterLine", e);
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value = "/water/waterline/remove.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody List<AjaxResponse> deleteWaterLine(@RequestParam String[] listId, HttpServletRequest request,
		...
		if (listId != null && listId.length > 0) {
			for (int i = 0; i < listId.length; i++) {
				...
				try {
					MbasePoint<WaterLine> waterline = waterlineService.getById(id);
					if (waterline != null) {
						waterlineService.delete(waterline);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/waterline/exportCols.shtml", method={RequestMethod.GET,RequestMethod.POST}, produces="application/json;charset=UTF-8")
	public @ResponseBody void  exportCols(Model model, @RequestParam(value="ebegindt", required=false) String begindt, @RequestParam(value="eendtm", required=false) String endtm, 
		...
		try {
			...
			if (eid != null ) {
				...
				where.add("id");
				where.add(eid);
				Entitites<MbasePoint<WaterLine>> list  = waterlineService.getPageListByAttributesLike(null, null, null, null, null,where,null);
				
				XSSFWorkbook wb=null;
				if(list!=null && !list.getEntites().isEmpty()){
					String fileName ="管线数据";
					
					for(MbasePoint<WaterLine> p:list.getEntites()){
						...
						include_where = new StringBuffer().append(" where ( CALIBRATION BETWEEN '").append(begindt+" 00:00:00").append("' AND '").append(endtm+" 23:59:59").append("' ) AND WATERLINE_ID = ").append(p.getId());
						List<Object[]> bbtype= waterlineService.getBySql(attributes, "WATERLINEDATA", include_where.toString());
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/waterline/importCols.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		...
		    		try{
		    			...
		    			try{
		    				...
		    				MbasePoint<WaterLine> msurface =waterlineService.getByNO(cedian, MPointEnumType.WaterLine,Long.parseLong(iid));
		    				WaterLine surface =(WaterLine) msurface;
		    				Date date =null;
		    				if(surface!=null){
		    					//日期是否存在，如果不存在即为当前的
			    				...
			    				WaterLineData pdata = new WaterLineData();
			    				...
			    				MDbasePoint<WaterLineData> last = waterlinedataService.getLast(date, MPointEnumType.WaterLine, surface.getId());
			    				MDbasePoint<WaterLineData> next = waterlinedataService.getNext(date, MPointEnumType.WaterLine, surface.getId());
			    				if(last!=null){
			    					pdata.setInitHeight(last.getCurtHeight());
			    				}
			    				if(next!=null){
			    					next.setInitHeight(current);
			    					waterlinedataService.update(next);
			    				}
			    				waterlinedataService.save(pdata);
		    				}else{
		    					returns.add(String.valueOf(i+1));
				    			float num= (float)i/trLength;  
				    			session.setAttribute("import", df.format(num));
	...
	@SuppressWarnings("unchecked")
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/waterline/importPoints.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
	public @ResponseBody String importPoints(Model model, @RequestParam(required=false, value="uploadfile") CommonsMultipartFile uploadfile, HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		if(!StringUtils.isBlank(cols) && StringUtils.isNotBlank(iid)){
			try {
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		Row row = sheet.getRow(i);
		    		WaterLine building =new WaterLine();
		    		try{
		    			//0:测点 1：本次高程 2：初始高程
		    			//0:1必须存在
		    			if(StringUtils.isBlank(transExcelView.getValue(row,col[0], true)) ||
		    		...
		    		waterlineService.save(building);
		    	}	
