package com.kekeinfo.web.admin.controller.displacement;
...
public class DisplacementController {
	private static final Logger LOGGER = LoggerFactory.getLogger(DisplacementController.class);
	@Autowired
	...
	@RequestMapping(value = "/water/displacement/list.html", method = RequestMethod.GET)
	public String listdisplacements(Model model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		return "admin-displacement";
	}

	@PreAuthorize("hasRole('VIEW-PROJECT')")
	@RequestMapping(value = "/water/displacement/server_processing.shtml", method = { RequestMethod.GET,
			RequestMethod.POST }, produces = " application/json; charset=utf-8")
	public @ResponseBody String getDisplacement(@RequestParam String aoData, HttpServletRequest request,
			HttpServletResponse response) throws JsonGenerationException, JsonMappingException, IOException {
		AjaxResponse resp = new AjaxResponse();
		DataTableParameter dataTableParam = new DataTableParameter(aoData);
		DataTable<MbasePoint<Displacement>> dt = new DataTable<MbasePoint<Displacement>>();
		String mid = request.getParameter("mid");
		try { // 指定根据什么条件进行模糊查询
			...
			Entitites<MbasePoint<Displacement>> list = displacementService.getPageListByAttributesLike(attributes, dataTableParam.getsSearch(), Long.valueOf(dataTableParam.getiDisplayLength()).intValue(), 
			...
			LOGGER.error("Error while paging displacements", e);
	...
	@RequestMapping(value = "/water/displacement/save.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody String saveDisplacement(@ModelAttribute("displacement") Displacement displacement, Model model,
			HttpServletRequest request, HttpServletResponse response) {
		AjaxResponse resp = new AjaxResponse();
		try {
			displacementService.saveOrUpdate(displacement);
			MbasePoint<Displacement> bdata =  displacementService.getById(displacement.getId(), MPointEnumType.Displacement);
			MDbasePoint<DisplacementData> supaxialdata = displacementdataService.getEqualsHeightData(bdata.getInitHeight(), displacement.getId(), MPointEnumType.Displacement);
			if(supaxialdata!=null) {
				supaxialdata.setInitHeight(displacement.getInitHeight());
				displacementdataService.update(supaxialdata);
			}
		} catch (ServiceException e) {
			LOGGER.error("Error while save Displacement", e);
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/displacement/exportCols.shtml", method={RequestMethod.GET,RequestMethod.POST}, produces="application/json;charset=UTF-8")
	public @ResponseBody void  exportCols(Model model, @RequestParam(value="ebegindt", required=false) String begindt, @RequestParam(value="eendtm", required=false) String endtm, 
			...
			try {
					...
					if (eid != null ) {
						...
						where.add("id");
						where.add(eid);
						Entitites<MbasePoint<Displacement>> list  = displacementService.getPageListByAttributesLike(null, null, null, null, null,where,null);
						
						XSSFWorkbook wb=null;
						if(list!=null && !list.getEntites().isEmpty()){
							String fileName ="水平位移数据";
							
							for(MbasePoint<Displacement> p:list.getEntites()){
								...
								StringBuffer include_where = new StringBuffer().append(" where ( DATE_CREATED BETWEEN '").append(begindt+" 00:00:00").append("' AND '").append(endtm+" 23:59:59").append("' ) AND DISPLACEMENT_ID = ").append(p.getId());
								List<Object[]> bbtype= displacementService.getBySql(attributes, "DISPLACEMENTDATA", include_where.toString());
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value = "/water/displacement/remove.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody List<AjaxResponse> deleteDisplacement(@RequestParam String[] listId,
		...
		if (listId != null && listId.length > 0) {
			for (int i = 0; i < listId.length; i++) {
				...
				try {
					MbasePoint<Displacement> displacement = displacementService.getById(id);
					if (displacement != null) {
						displacementService.delete(displacement);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/displacement/importCols.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		...
		    		try{
		    			...
		    			try{
		    				...
		    				MbasePoint<Displacement> msurface =displacementService.getByNO(cedian, MPointEnumType.Displacement,Long.parseLong(iid));
		    				Displacement surface =(Displacement) msurface;
		    				Date date =null;
		    				if(surface!=null){
		    					//日期是否存在，如果不存在即为当前的
			    				...
			    				DisplacementData pdata = new DisplacementData();
			    				...
			    				MDbasePoint<DisplacementData> last = displacementdataService.getLast(date, MPointEnumType.Displacement, surface.getId());
			    				MDbasePoint<DisplacementData> next = displacementdataService.getNext(date, MPointEnumType.Displacement, surface.getId());
			    				if(last!=null){
			    					pdata.setInitHeight(last.getCurtHeight());
			    				}
			    				if(next!=null){
			    					next.setInitHeight(current);
			    					displacementdataService.update(next);
			    				}
			    				displacementdataService.save(pdata);
		    				}else{
		    					returns.add(String.valueOf(i+1));
				    			float num= (float)i/trLength;  
				    			session.setAttribute("import", df.format(num));
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/displacement/importPoints.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
	public @ResponseBody String importPoints(Model model, @RequestParam(required=false, value="uploadfile") CommonsMultipartFile uploadfile, HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		if(!StringUtils.isBlank(cols) && StringUtils.isNotBlank(iid)){
			try {
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		Row row = sheet.getRow(i);
		    		Displacement building =new Displacement();
		    		try{
		    			//0:测点 1：本次高程 2：初始高程
		    			//0:1必须存在
		    			if(StringUtils.isBlank(transExcelView.getValue(row,col[0], true)) ||
		    		...
		    		displacementService.save(building);
		    	}	
	...
	private void setMenu(Model model, HttpServletRequest request) throws Exception {
		...
		activeMenus.put("displacement", "displacement");
		activeMenus.put("displacement-list", "displacement-list");
		@SuppressWarnings("unchecked")
		Map<String, Menu> menus = (Map<String, Menu>) request.getAttribute("MENUMAP");
		Menu currentMenu = (Menu) menus.get("displacement");
