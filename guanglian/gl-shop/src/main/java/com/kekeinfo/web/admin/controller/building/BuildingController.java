package com.kekeinfo.web.admin.controller.building;
...
public class BuildingController {
	...
	@RequestMapping(value = "/water/building/list.html", method = RequestMethod.GET)
	public String listbuildings(Model model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
	...
	@PreAuthorize("hasRole('VIEW-PROJECT')")
	@RequestMapping(value = "/water/building/server_processing.shtml", method = { RequestMethod.GET,
	...
	public @ResponseBody String getBuilding(@RequestParam String aoData, HttpServletRequest request,
		...
		DataTable<MbasePoint<Building>> dt = new DataTable<MbasePoint<Building>>();
		String mid = request.getParameter("mid");
		try { // 指定根据什么条件进行模糊查询
			...
			Entitites<MbasePoint<Building>> list = buildingService.getPageListByAttributesLike(attributes, dataTableParam.getsSearch(), Long.valueOf(dataTableParam.getiDisplayLength()).intValue(), 
	...
	@RequestMapping(value = "/water/building/save.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody String saveBuilding(@ModelAttribute("building") Building building, Model model,
			HttpServletRequest request, HttpServletResponse response) {
		AjaxResponse resp = new AjaxResponse();
		try {
			buildingService.saveOrUpdate(building);
				MbasePoint<Building> bdata =  buildingService.getById(building.getId(), MPointEnumType.Building);
	            MDbasePoint<BuildingData> buildingdata = buildingdataService.getEqualsHeightData(bdata.getInitHeight(), building.getId(), MPointEnumType.Building);
				if(buildingdata!=null) {
					buildingdata.setInitHeight(building.getInitHeight());
					buildingdataService.update(buildingdata);
				}
		} catch (ServiceException e) {
			LOGGER.error("Error while save Building", e);
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/building/exportCols.shtml", method={RequestMethod.GET,RequestMethod.POST}, produces="application/json;charset=UTF-8")
	public @ResponseBody void  exportCols(Model model, @RequestParam(value="ebegindt", required=false) String begindt, @RequestParam(value="eendtm", required=false) String endtm, 
		...
		try {
			...
			if (eid != null ) {
				...
				where.add("id");
				where.add(eid);
				Entitites<MbasePoint<Building>> list  = buildingService.getPageListByAttributesLike(null, null, null, null, null,where,null);
				
				XSSFWorkbook wb=null;
				if(list!=null && !list.getEntites().isEmpty()){
					String fileName ="建筑物数据";
					
					
					for(MbasePoint<Building> p:list.getEntites()){
						...
						include_where = new StringBuffer().append(" where ( CALIBRATION BETWEEN '").append(begindt+" 00:00:00").append("' AND '").append(endtm+" 23:59:59").append("' ) AND BUILDING_ID = ").append(p.getId());
						List<Object[]> bbtype= buildingService.getBySql(attributes, "BUILDINGDATA", include_where.toString());
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/building/importCols.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		...
		    		try{
		    			...
		    			try{
		    				...
		    				MbasePoint<Building> msurface =buildingService.getByNO(cedian, MPointEnumType.Building,Long.parseLong(iid));
		    				Building surface =(Building) msurface;
		    				Date date =null;
		    				if(surface!=null){
		    					//日期是否存在，如果不存在即为当前的
			    				...
			    				MDbasePoint<BuildingData> last = buildingdataService.getLast(date, MPointEnumType.Building, surface.getId());
			    				MDbasePoint<BuildingData> next = buildingdataService.getNext(date, MPointEnumType.Building, surface.getId());
			    				if(last!=null){
			    					pdata.setInitHeight(last.getCurtHeight());
			    				}
			    				if(next!=null){
			    					next.setInitHeight(current);
			    					buildingdataService.update(next);
			    				}
			    				buildingdataService.save(pdata);
		    				}else{
		    					returns.add(String.valueOf(i+1));
				    			float num= (float)i/trLength;  
				    			session.setAttribute("import", df.format(num));
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/building/importPoints.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value = "/water/building/remove.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody List<AjaxResponse> deleteBuilding(@RequestParam String[] listId, HttpServletRequest request,
		...
		if (listId != null && listId.length > 0) {
			for (int i = 0; i < listId.length; i++) {
				...
				try {
					MbasePoint<Building> building = buildingService.getById(id);
					if (building != null) {
	...
	private void setMenu(Model model, HttpServletRequest request) throws Exception {
		...
		activeMenus.put("building", "building");
		activeMenus.put("building-list", "building-list");
		@SuppressWarnings("unchecked")
		Map<String, Menu> menus = (Map<String, Menu>) request.getAttribute("MENUMAP");
		Menu currentMenu = (Menu) menus.get("building");
