package com.kekeinfo.web.admin.controller.dewateringdata;
...
@Controller
public class DeWellDataController {
	private static final Logger LOGGER = LoggerFactory.getLogger(DeWellDataController.class);
	...
	@RequestMapping(value="/water/dedata/list.html", method=RequestMethod.GET)
	public String display(Model model, HttpServletRequest request, HttpServletResponse response, Locale locale) throws Exception {
		...
		if(users!=null){
			...
			if(!StringUtils.isBlank(pID)){
				try{
					model.addAttribute("welltype", "疏干井");
					Dewatering pwell = dewateringService.getByIdWithCSite(Long.parseLong(pID));
					...
					if(pwell.getPointInfo().getImages()!=null && pwell.getPointInfo().getImages().size()>0){
						...
						for(Wellimages im:pwell.getPointInfo().getImages()){
							imgs.add(request.getContextPath() + ImageFilePathUtils.buildFilePathByConentType(im.getJpeg(), FileContentType.DEWELL_DIGITAL));
						}
	...
	@PreAuthorize("hasRole('VIEW-PROJECT')")
	@RequestMapping(value = "/water/dedata/server_processing.shtml", method = { RequestMethod.GET, RequestMethod.POST }, produces = "application/json; charset=utf-8")
	public @ResponseBody
	...
	@PreAuthorize("hasRole('VIEW-PROJECT')")
	@RequestMapping(value = "/water/dedata/lastAccu.shtml", method =  RequestMethod.POST , produces = "application/json; charset=utf-8")
	public @ResponseBody String getLast(HttpServletRequest request,HttpServletResponse response) {
		String csiteID = request.getParameter("pid");
		if(!StringUtils.isBlank(csiteID)){
			Dewatering pwell = dewateringService.getById(Long.parseLong(csiteID));
			if(pwell!=null && pwell.getThisAccu()!=null){
				try {
					Map<String, Object> entry = new HashMap<String, Object>();
	...
	@RequestMapping(value="/water/dedata/savemap.shtml", method=RequestMethod.POST, produces="application/json; charset=utf-8")
		...
		if((!StringUtils.isBlank(water) || !StringUtils.isBlank(flow)) && StringUtils.isNotBlank(pid)){
			try {
				...
				if(user!=null){
					...
					//上次累计
					if(pData.getLastAccu()==null){
						pData.setLastAccu(new BigDecimal(0));
					}
					hdeWellDataService.saveOrUpdate(pData, pwell,pname,auses);
					pnodeUtils.OpenWithDeep(pwell.getId(), pData,PointEnumType.DEWATERING);
					//更新缓存
	...
	@SuppressWarnings("unchecked")
	@PreAuthorize("hasRole('EDIT-PROJECT')") 
	@RequestMapping(value="/water/dedata/save.shtml", method=RequestMethod.POST, produces="application/json; charset=utf-8")
	public @ResponseBody String save(@ModelAttribute("csite") HdewellData pData, Model model, HttpServletRequest request, HttpServletResponse response)  {
		AjaxResponse resp = new AjaxResponse();
		...
		try {
			...
			if(user!=null){
				...
				pData.getAuditSection().setModifiedBy(user.getFirstName());
				//增加阈值数据
				Dewatering pwell = dewateringService.getByIdWithCSite(pData.getDeWell().getId());
				int status = 0;
				if(pData.getFlow()!=null){
					if( pData.getFlow().compareTo(pwell.getFlow())==1){
						status=1;
	...
	@SuppressWarnings("unchecked")
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/dedata/importCols.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
			...
			try {
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		...
		    		try{
		    			...
		    			hdeWellDataService.saveOrUpdate(pdata, pwell,pname,auses); //测点的最新状态，统一上传后再处理
		    		}catch (Exception e){
		    			e.printStackTrace();
		    			returns.add(String.valueOf(i+1));
		    			float num= (float)i/trLength;  
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/dedata/exportCols.shtml", method={RequestMethod.GET,RequestMethod.POST}, produces="application/json;charset=UTF-8")
	public @ResponseBody void  exportCols(Model model, @RequestParam(value="ebegindt", required=false) String begindt, @RequestParam(value="eendtm", required=false) String endtm, HttpServletRequest request, HttpServletResponse response, Locale locale) throws IOException {
		...
		resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);*/
			try {
				...
				StringBuffer include_where = new StringBuffer().append(" where ( DATE_CREATED BETWEEN '").append(begindt).append("' AND '").append(endtm).append("' ) AND DEWELL_ID = ").append(pid);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/dedata/remove.shtml", method=RequestMethod.POST, produces="application/json;charset=utf-8")
	public @ResponseBody List<AjaxResponse> remove(@RequestParam String[] listId,HttpServletRequest request, Locale locale) throws Exception {
		...
		if (listId!=null&&listId.length>0) {
			...
			for (int i = 0; i < listId.length; i++) {
				...
				try {
					csiteid = hdeWellDataService.deleteBydId(id);
					resp.setStatus(AjaxResponse.RESPONSE_STATUS_SUCCESS);
					resplist.add(resp);
				} catch (Exception e) {
					e.printStackTrace();
