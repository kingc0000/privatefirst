package com.kekeinfo.web.admin.controller.ringbeam;
...
@Controller
public class RingBeamController {
	private static final Logger LOGGER = LoggerFactory.getLogger(RingBeamController.class);
	@Autowired
	RingBeamService ringbeamService;
	@Autowired
	...
	@RequestMapping(value = "/water/ringbeam/list.html", method = RequestMethod.GET)
	public String listringbeams(Model model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String mid = request.getParameter("mid");
		setMenu(model, request);
		RingBeam ringbeam = new RingBeam();
		model.addAttribute("ringbeam", ringbeam);
		//判断用户是否有该项目的编辑权限
		...
		return "admin-ringbeam";
	}

	@PreAuthorize("hasRole('VIEW-PROJECT')")
	@RequestMapping(value = "/water/ringbeam/server_processing.shtml", method = { RequestMethod.GET,
			RequestMethod.POST }, produces = " application/json; charset=utf-8")
	public @ResponseBody String getRingBeam(@RequestParam String aoData, HttpServletRequest request,
			HttpServletResponse response) throws JsonGenerationException, JsonMappingException, IOException {
		AjaxResponse resp = new AjaxResponse();
		DataTableParameter dataTableParam = new DataTableParameter(aoData);
		DataTable<MbasePoint<RingBeam>> dt = new DataTable<MbasePoint<RingBeam>>();
		String mid = request.getParameter("mid");
		try { // 指定根据什么条件进行模糊查询
			...
			Entitites<MbasePoint<RingBeam>> list = ringbeamService.getPageListByAttributesLike(attributes,
			...
			LOGGER.error("Error while paging ringbeams", e);
	...
	@RequestMapping(value = "/water/ringbeam/save.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody String saveRingBeam(@ModelAttribute("ringbeam") RingBeam ringbeam, Model model,
			HttpServletRequest request, HttpServletResponse response) {
		AjaxResponse resp = new AjaxResponse();
		try {
			ringbeamService.saveOrUpdate(ringbeam);
				MbasePoint<RingBeam> bdata =  ringbeamService.getById(ringbeam.getId(), MPointEnumType.RingBeam);
				MDbasePoint<RingBeamData> ringbeamdata = ringbeamdataService.getEqualsHeightData(bdata.getInitHeight(), ringbeam.getId(), MPointEnumType.RingBeam);
				if(ringbeamdata!=null) {
					ringbeamdata.setInitHeight(ringbeam.getInitHeight());
					ringbeamdataService.update(ringbeamdata);
				}
		} catch (ServiceException e) {
			LOGGER.error("Error while save RingBeam", e);
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/ringbeam/exportCols.shtml", method={RequestMethod.GET,RequestMethod.POST}, produces="application/json;charset=UTF-8")
	public @ResponseBody void  exportCols(Model model, @RequestParam(value="ebegindt", required=false) String begindt, @RequestParam(value="eendtm", required=false) String endtm, 
		...
		try {
			...
			if (eid != null ) {
				...
				where.add("id");
				where.add(eid);
				Entitites<MbasePoint<RingBeam>> list  = ringbeamService.getPageListByAttributesLike(null, null, null, null, null,where,null);
				
				XSSFWorkbook wb=null;
				if(list!=null && !list.getEntites().isEmpty()){
					String fileName ="圈梁数据";
					
					
					for(MbasePoint<RingBeam> p:list.getEntites()){
						...
						include_where = new StringBuffer().append(" where ( CALIBRATION BETWEEN '").append(begindt+" 00:00:00").append("' AND '").append(endtm+" 23:59:59").append("' ) AND RINGBEAM_ID = ").append(p.getId());
						List<Object[]> bbtype= ringbeamService.getBySql(attributes, "RINGBEAMDATA", include_where.toString());
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value = "/water/ringbeam/remove.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody List<AjaxResponse> deleteRingBeam(@RequestParam String[] listId, HttpServletRequest request,
		...
		if (listId != null && listId.length > 0) {
			for (int i = 0; i < listId.length; i++) {
				...
				try {
					MbasePoint<RingBeam> ringbeam = ringbeamService.getById(id);
					if (ringbeam != null) {
						ringbeamService.delete(ringbeam);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/ringbeam/importCols.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		...
		    		try{
		    			...
		    			try{
		    				...
		    				MbasePoint<RingBeam> msurface =ringbeamService.getByNO(cedian, MPointEnumType.RingBeam,Long.parseLong(iid));
		    				RingBeam surface =(RingBeam) msurface;
		    				Date date =null;
		    				if(surface!=null){
		    					//日期是否存在，如果不存在即为当前的
			    				...
			    				RingBeamData pdata = new RingBeamData();
			    				...
			    				MDbasePoint<RingBeamData> last = ringbeamdataService.getLast(date, MPointEnumType.RingBeam, surface.getId());
			    				MDbasePoint<RingBeamData> next = ringbeamdataService.getNext(date, MPointEnumType.RingBeam, surface.getId());
			    				if(last!=null){
			    					pdata.setInitHeight(last.getCurtHeight());
			    				}
			    				if(next!=null){
			    					next.setInitHeight(current);
			    					ringbeamdataService.update(next);
			    				}
			    				ringbeamdataService.save(pdata);
		    				}else{
		    					returns.add(String.valueOf(i+1));
				    			float num= (float)i/trLength;  
				    			session.setAttribute("import", df.format(num));
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/ringbeam/importPoints.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
	public @ResponseBody String importPoints(Model model, @RequestParam(required=false, value="uploadfile") CommonsMultipartFile uploadfile, HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		if(!StringUtils.isBlank(cols) && StringUtils.isNotBlank(iid)){
			try {
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		Row row = sheet.getRow(i);
		    		RingBeam building =new RingBeam();
		    		try{
		    			//0:测点 1：本次高程 2：初始高程
		    			//0:1必须存在
		    			if(StringUtils.isBlank(transExcelView.getValue(row,col[0], true)) ||
	...
	private void setMenu(Model model, HttpServletRequest request) throws Exception {
		...
		activeMenus.put("ringbeam", "ringbeam");
		activeMenus.put("ringbeam-list", "ringbeam-list");
		@SuppressWarnings("unchecked")
		Map<String, Menu> menus = (Map<String, Menu>) request.getAttribute("MENUMAP");
		Menu currentMenu = (Menu) menus.get("ringbeam");
