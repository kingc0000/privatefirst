package com.kekeinfo.web.admin.controller.hiddenline;
...
@Controller
public class HiddenLineController {
	private static final Logger LOGGER = LoggerFactory.getLogger(HiddenLineController.class);
	@Autowired
	HiddenLineService hiddenlineService;
	@Autowired
	...
	@RequestMapping(value = "/water/hiddenline/list.html", method = RequestMethod.GET)
	public String listhiddenlines(Model model, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		String mid = request.getParameter("mid");
		setMenu(model, request);
		HiddenLine hiddenline = new HiddenLine();
		model.addAttribute("hiddenline", hiddenline);
		//判断用户是否有该项目的编辑权限
		...
		return "admin-hiddenline";
	}

	@PreAuthorize("hasRole('VIEW-PROJECT')")
	@RequestMapping(value = "/water/hiddenline/server_processing.shtml", method = { RequestMethod.GET,
			RequestMethod.POST }, produces = " application/json; charset=utf-8")
	public @ResponseBody String getHiddenLine(@RequestParam String aoData, HttpServletRequest request,
			HttpServletResponse response) throws JsonGenerationException, JsonMappingException, IOException {
		AjaxResponse resp = new AjaxResponse();
		DataTableParameter dataTableParam = new DataTableParameter(aoData);
		DataTable<MbasePoint<HiddenLine>> dt = new DataTable<MbasePoint<HiddenLine>>();
		String mid = request.getParameter("mid");
		try { // 指定根据什么条件进行模糊查询
			...
			Entitites<MbasePoint<HiddenLine>> list = hiddenlineService.getPageListByAttributesLike(attributes,
			...
			LOGGER.error("Error while paging hiddenlines", e);
	...
	@RequestMapping(value = "/water/hiddenline/save.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody String saveHiddenLine(@ModelAttribute("hiddenline") HiddenLine hiddenline, Model model,
			HttpServletRequest request, HttpServletResponse response) {
		AjaxResponse resp = new AjaxResponse();
		try {
			hiddenlineService.saveOrUpdate(hiddenline);
				MbasePoint<HiddenLine> bdata =  hiddenlineService.getById(hiddenline.getId(), MPointEnumType.HiddenLine);
				MDbasePoint<HiddenLineData> buildingdata = hiddenlinedataService.getEqualsHeightData(bdata.getInitHeight(), hiddenline.getId(), MPointEnumType.HiddenLine);
				if(buildingdata!=null) {
					buildingdata.setInitHeight(hiddenline.getInitHeight());
					hiddenlinedataService.update(buildingdata);
				}
		} catch (ServiceException e) {
			LOGGER.error("Error while save HiddenLine", e);
			resp.setStatus(AjaxResponse.RESPONSE_STATUS_FAIURE);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/hiddenline/exportCols.shtml", method={RequestMethod.GET,RequestMethod.POST}, produces="application/json;charset=UTF-8")
	public @ResponseBody void  exportCols(Model model, @RequestParam(value="ebegindt", required=false) String begindt, @RequestParam(value="eendtm", required=false) String endtm, 
		...
		try {
			...
			if (eid != null ) {
				...
				where.add("id");
				where.add(eid);
				Entitites<MbasePoint<HiddenLine>> list  = hiddenlineService.getPageListByAttributesLike(null, null, null, null, null,where,null);
				
				XSSFWorkbook wb=null;
				if(list!=null && !list.getEntites().isEmpty()){
					String fileName ="潜层水位数据";
					
					for(MbasePoint<HiddenLine> p:list.getEntites()){
						...
						include_where = new StringBuffer().append(" where ( CALIBRATION BETWEEN '").append(begindt+" 00:00:00").append("' AND '").append(endtm+" 23:59:59").append("' ) AND HIDDENLINE_ID = ").append(p.getId());
						List<Object[]> bbtype= hiddenlineService.getBySql(attributes, "HIDDENLINEDATA", include_where.toString());
						String[] title={"上次水位","本次水位","总累计位移","校准时间"};
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value = "/water/hiddenline/remove.shtml", method = RequestMethod.POST, produces = "application/json; charset=utf-8")
	public @ResponseBody List<AjaxResponse> deleteHiddenLine(@RequestParam String[] listId, HttpServletRequest request,
		...
		if (listId != null && listId.length > 0) {
			for (int i = 0; i < listId.length; i++) {
				...
				try {
					MbasePoint<HiddenLine> hiddenline = hiddenlineService.getById(id);
					if (hiddenline != null) {
						hiddenlineService.delete(hiddenline);
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/hiddenline/importCols.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		...
		    		try{
		    			...
		    			try{
		    				...
		    				MbasePoint<HiddenLine> msurface =hiddenlineService.getByNO(cedian, MPointEnumType.HiddenLine,Long.parseLong(iid));
		    				HiddenLine surface =(HiddenLine) msurface;
		    				Date date =null;
		    				if(surface!=null){
		    					//日期是否存在，如果不存在即为当前的
			    				...
			    				HiddenLineData pdata = new HiddenLineData();
			    				...
			    				MDbasePoint<HiddenLineData> last = hiddenlinedataService.getLast(date, MPointEnumType.HiddenLine, surface.getId());
			    				MDbasePoint<HiddenLineData> next = hiddenlinedataService.getNext(date, MPointEnumType.HiddenLine, surface.getId());
			    				if(last!=null){
			    					pdata.setInitHeight(last.getCurtHeight());
			    				}
			    				if(next!=null){
			    					next.setInitHeight(current);
			    					hiddenlinedataService.update(next);
			    				}
			    				hiddenlinedataService.save(pdata);
		    				}else{
		    					returns.add(String.valueOf(i+1));
				    			float num= (float)i/trLength;  
				    			session.setAttribute("import", df.format(num));
	...
	@PreAuthorize("hasRole('EDIT-PROJECT')")
	@RequestMapping(value="/water/hiddenline/importPoints.shtml", method=RequestMethod.POST, produces="text/html;charset=UTF-8")
	public @ResponseBody String importPoints(Model model, @RequestParam(required=false, value="uploadfile") CommonsMultipartFile uploadfile, HttpServletRequest request, HttpServletResponse response, Locale locale) {
		...
		if(!StringUtils.isBlank(cols) && StringUtils.isNotBlank(iid)){
			try {
		    	...
		    	for(int i=0;i<=trLength;i++){
		    		Row row = sheet.getRow(i);
		    		HiddenLine building =new HiddenLine();
		    		try{
		    			//0:测点 1：本次高程 2：初始高程
		    			//0:1必须存在
		    			if(StringUtils.isBlank(transExcelView.getValue(row,col[0], true)) ||
		    		...
		    		hiddenlineService.save(building);
		    	}	
	...
	private void setMenu(Model model, HttpServletRequest request) throws Exception {
		...
		activeMenus.put("hiddenline", "hiddenline");
		activeMenus.put("hiddenline-list", "hiddenline-list");
		@SuppressWarnings("unchecked")
		Map<String, Menu> menus = (Map<String, Menu>) request.getAttribute("MENUMAP");
		Menu currentMenu = (Menu) menus.get("hiddenline");
