package com.kekeinfo.core.business.pumpwell.model;
...
@Table(name = "PUMPWELL", schema=SchemaConstant.KEKEINFO_SCHEMA)
public class Pumpwell extends Basepoint<PumpLink, PumpInfo> implements Auditable {
	...
	@Id
	@Column(name = "PUMPWELL_ID", nullable=false)
	@TableGenerator(name = "TABLE_GEN", table = "SM_SEQUENCER", pkColumnName = "SEQ_NAME", valueColumnName = "SEQ_COUNT", pkColumnValue = "PUMPWELL_SEQ_NEXT_VAL")
	...
	public void updateDataStatusByData(BigDecimal flow, BigDecimal water) {
		int status = 0;
		if(flow!=null && getFlow()!=null){
			if(flow.compareTo(getFlow())==1){
				status=1;
			}
		}
		
		if(water!=null && getWater()!=null && getWaterDwon()!=null){
