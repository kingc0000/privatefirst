...
//    private short nextTransactionId = 0;
    private static int nextTransactionId = 5377; //0x15 0x01开头
    private static int maxTransactionId = 65536; //0xff 0xff
    ...
    public TcpMaster(Socket socket, boolean keepAlive) {
        this.socket = socket;
        this.keepAlive = keepAlive;
    }
    ...
    @Override
    synchronized public void destroyWithoutSocket() {
        closeConnectionWithoutSocket();
        initialized = false;
    }
        ...
        try {
            ...
            if (keepAlive) {
                ...
                try {
                    ...
                    ipResponse = (IpMessageResponse) conn.send(ipRequest);
                    if (ipResponse == null) {
                    	this.closeConnectionWithoutSocket();
                    	return null;
                    }
                    if(LOG.isDebugEnabled()){
            	    	StringBuilder sb = new StringBuilder();
    ...
    private void openConnection() throws IOException {
        ...
        LOG.debug("创建新的messagecontrol, timeout="+conn.getTimeout());
    ...
    private void closeConnectionWithoutSocket() {
    	closeMessageControl(conn);
        conn = null;
        socket = null;
    }
