...
import com.serotonin.modbus4j.ip.xa.HandWaitingRoomKeyFactory;
...
public class TcpHandMaster extends ModbusMaster {
    private static final int RETRY_PAUSE_START = 50;
    private static final int RETRY_PAUSE_MAX = 1000;

    // Configuration fields.
	private final Log LOG = LogFactory.getLog(TcpHandMaster.class);
    //private short nextTransactionId = 0;
	private short nextTransactionId = 5376; //0x15 0x00开头
    ...
    public TcpHandMaster(IpParameters params, boolean keepAlive) {
        ipParameters = params;
    ...
    synchronized public ModbusResponse sendImpl(ModbusRequest request) throws ModbusTransportException {
    	LOG.debug("request functionCode=" + String.format("%02X ", request.getFunctionCode()));
    	try {
        ...
        ipRequest = new HandMessageRequest(request, getNextTransactionId());
    ...
    private void openConnection() throws IOException {
        ...
        ipMessageParser = new HandMessageParser(true);
        waitingRoomKeyFactory = new HandWaitingRoomKeyFactory();
        
        conn = getHandMessageControl();
        conn.start(transport, ipMessageParser, null, waitingRoomKeyFactory);
        if (LOG.isDebugEnabled()) { //conn 设置为debug模式
			conn.DEBUG = true;
		}
        if (getePoll() == null)
            ((StreamTransport) transport).start("Modbus4J TcpHandMaster");
    }

    private void closeConnection() {
    	closeHandMessageControl(conn);
        try {
            if (socket != null)
                socket.close();
        }
