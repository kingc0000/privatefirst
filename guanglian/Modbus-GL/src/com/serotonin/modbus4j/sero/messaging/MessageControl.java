...
public class MessageControl implements DataConsumer {
    ...
    private final Log LOG = LogFactory.getLog(MessageControl.class);
    ...
    public IncomingResponseMessage send(OutgoingRequestMessage request, int timeout, int retries) throws IOException {
        ...
        if (request.expectsResponse()) {
            ...
            LOG.info("创建waitingRoomKey with request ="+key.hashCode()+" "+key.toString()+" waitingroom="+waitingRoom);
            ...
            try {
                do {
                    ...
                    if (response == null)
                        LOG.debug("Timeout waiting for response");
                }
                while (response == null && retries-- > 0);
            } catch (WaitingRoomException e) {
            	this.close(); //移除consumer
				e.printStackTrace();
			}
            finally {
    ...
    public void data(byte[] b, int len) {
        LOG.debug("MessagingControl.read: " + StreamUtils.dumpHex(b, 0, len));
        if (ioLog != null)
            ioLog.input(b, 0, len);
        ...
        while (true) {
            ...
            try {
            	...
            	LOG.error("remove consumer, this="+this.hashCode());
            	this.close(); //移除consumer
