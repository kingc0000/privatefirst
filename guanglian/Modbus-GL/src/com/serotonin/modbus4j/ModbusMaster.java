...
abstract public class ModbusMaster extends Modbus {
    ...
    private final Log LOG = LogFactory.getLog(ModbusMaster.class);
    ...
    public void destroyWithoutSocket() {
    ...
    protected HandMessageControl getHandMessageControl() {
    	HandMessageControl conn = new HandMessageControl();
        conn.setRetries(getRetries());
        conn.setTimeout(getTimeout());
        conn.setDiscardDataDelay(getDiscardDataDelay());
        conn.setExceptionHandler(getExceptionHandler());
    ...
    protected void closeHandMessageControl(HandMessageControl conn) {
        if (conn != null)
            conn.close();
    }    //
    //
    ...
    private <K> void sendFunctionGroup(ReadFunctionGroup<K> functionGroup, BatchResults<K> results,
        ...
        LOG.debug("MessagingControl.response.getData(): " + StreamUtils.dumpHex(data));
        for (KeyedModbusLocator<K> locator : functionGroup.getLocators()) {
            if (errorsInResults && response.isException())
